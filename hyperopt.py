...
def objective(trial):
    ...
    n_layers = 2
    hidden_sizes = [trial.suggest_int(f'hidden_size_{layer}', 16, 128, log = True) for layer in range(1,n_layers+1)]
    params = {
        'Base_dnn_hidden_sizes': hidden_sizes,
        'Base_lstm_hidden_size':trial.suggest_int('lstm_hidden_size', 16, 128, log = True),
        'mfe_loss_weight': trial.suggest_float('mfe_loss_weight', 0.01, 10, log = True),
